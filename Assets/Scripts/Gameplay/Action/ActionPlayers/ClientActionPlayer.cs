...
namespace Unity.BossRoom.Gameplay.Actions
{
    ...
    public sealed class ClientActionPlayer
    {
        ...
        /// </remarks>
        /// <remarks>
        /// 액션 예상(Anticipation)이란 무엇이며 어떤 문제를 해결하는가? 간단히 말하면, 액션을 실행시키는 입력 이벤트가 로컬 클라이언트에서
        /// 감지되는 순간 액션의 로직이 실행되도록 합니다. 이는 지연(latency)을 마스킹하는 데 도움을 주기 위한 것입니다. 이 데모는 서버가 권한을 가지므로,
        /// 기본 동작은 서버-클라이언트 왕복 후에야 입력에 대한 피드백을 보는 것입니다. 왕복 지연이 200ms를 넘으면, 이는 매우 느리게 느껴집니다.
        /// 이를 해결하기 위해, 시각 효과를 즉시 재생할 수 있습니다. 예를 들어, MeleeActionFX는 서버의 응답을 기다리지 않고 무기 휘두르기와 타격 반응을
        /// 모두 실행합니다. 이로 인해 서버가 목표물이 맞지 않았다고 생각할 수 있지만, 네트워크 상에서는 더 반응성이 좋아 보일 수 있습니다.
        ///
        /// 액션 예상은 기회주의적입니다—강력한 보장을 하지 않습니다. 예를 들어, 이미 애니메이션이 실행 중이면 예상된 액션 애니메이션을 얻을 수 없습니다.
        /// 또 다른 복잡성은 서버가 실제로 요청된 모든 액션을 실행하게 할지 알 수 없다는 점입니다. 일부 액션은 버려질 수 있습니다(예: 대기열에 너무 많은 액션이 있으면).
        /// 예상된 액션(구성되었지만 시작되지 않은 액션)은 서버에서 승인된 실제 액션과 완벽하게 일치하지 않을 수 있습니다. 따라서 PlayAction을 수신하고
        /// 예상된 액션이 이미 시작되지 않은 상태여도 괜찮습니다(서버와 다른 캐릭터에 대해 해당됩니다).
        ///
        /// 예상된 액션의 또 다른 중요한 측면은 "선택적(opt-in)" 시스템이라는 것입니다. Start 구현에서 base.Start를 호출해야 하지만, 
        /// 예상 효과를 구현할 방법이 없다면 아무 것도 할 필요는 없습니다. 이 경우 해당 액션은 "정상적으로" 재생됩니다(서버의 액션 방송이
        ...
        ///   2. ActionFX.AnticipateAction을 재정의합니다. base.AnticipateAction을 호출하고, 시각적 로직을 실행합니다(PlayAttackAnim과 같은).
