...
namespace Unity.BossRoom.VisualEffects
{
    ...
    /// </remarks>
    /// <summary>
    /// 특수 효과 프리팹에 첨부된 유틸리티 스크립트입니다. 이 프리팹은 "충전 중" 파티클, 지면 경로 표시기 등과 같은
    /// 짧은 시간 동안만 표시되는 특수 그래픽을 보여야 하는 다양한 ActionFX에 의해 사용됩니다.
    /// 
    /// 두 가지 다른 개념적 "모드"가 있습니다:
    /// - 누군가 명시적으로 Shutdown()을 호출할 때까지 계속 실행 (불확실한 기간을 가진 액션에서 사용; m_AutoShutdownTime을 -1로 설정)
    /// - 일정 시간이 지나면 자동으로 Shutdown()을 호출 (m_AutoShutdownTime을 초 단위로 설정)
    /// 
    /// 어떤 모드를 사용하든, Shutdown()은 중단된 액션의 경우 해당 그래픽을 소유한 사람이 미리 호출할 수 있습니다.
    /// 
    /// Shutdown()이 호출되면 (어쨌든) 파티클이 끝난 후 객체는 자동으로 파괴됩니다
    /// (또는 특정 추가 시간이 지난 후).
    /// </summary>
    /// <remarks>
    /// 파티클 시스템이 종료될 때, 마지막 남은 파티클들이 렌더링을 마칠 수 있도록 잠시 대기해야 하는 경우가 많습니다.
    /// Shutdown()은 파티클을 끄고, 모든 파티클이 사라진 후 자동으로 삭제됩니다. 파티클 시스템은 기술적으로
    /// 정지된 후 스스로 삭제될 수 있습니다: 파티클 시스템 검사기의 "Stop Action" 필드를 참조하십시오.
    /// 그러나 이 스크립트는 비파티클 그래픽도 자가 삭제하는 방법으로 작동하며, 객체 풀링을 구현하는 경우
    /// (모바일 성능 향상 목적) 이 클래스는 자가 삭제 대신 객체 풀로 자신을 이동하도록 리팩터링할 수 있습니다.
    /// </remarks>
